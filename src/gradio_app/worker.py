...
Gradio 主进程通过 ``python -m src.gradio_app.worker <payload.json>`` 启动本模块，
实现训练与 Web 进程隔离：主进程崩溃/重启不再杀死训练；训练崩溃不再影响 UI。
...
def main() -> int:
    if len(sys.argv) < 2:
        print("用法: python -m src.gradio_app.worker <payload.json>")
        return 2
