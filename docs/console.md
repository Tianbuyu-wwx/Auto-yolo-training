...
控制台能启停训练、上传并解压数据集、下载权重 —— 但它**没有认证层**：AYT 是给
个人下载后自己部署的训练器，不是多人共用的平台，所以不做账号 / 密钥体系。

安全边界因此只有一个：**监听地址**。

```bash
ayt-web                      # 默认 127.0.0.1:8080，仅本机可访问（推荐）
ayt-web --host 0.0.0.0       # 同网段任何人都能操作训练 —— 启动日志会显式警告
```

要对外提供访问，请把 `ayt-web` 放在反向代理（Nginx / Caddy，加 TLS 与 basic auth）
或 VPN / 隧道后面，而不是直接暴露端口。

!!! warning "别把「本机工具」当公网服务跑"
    没有认证层是设计选择，不是遗漏。绑到非回环地址时启动日志会明确写出后果：
    同一网络内任何人都能启停训练、上传数据集、下载权重。
