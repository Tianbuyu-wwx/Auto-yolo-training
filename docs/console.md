...
控制台能启停训练、上传并解压数据集、下载权重，因此它和推理服务共用同一把
API Key：读 `YOLO_API__API_KEY`（或旧名 `YOLO_API_KEY`），也可以用
`--api-key` 显式传入。

```bash
YOLO_API_KEY=your-strong-secret ayt-web --host 0.0.0.0 --port 8080
# 或
ayt-web --host 0.0.0.0 --port 8080 --api-key your-strong-secret
```

**未配置 key 时不校验任何请求**（沿用推理服务的语义，本地开发零摩擦）。
这时如果把服务绑到 `0.0.0.0`，同网段任何人都能操作训练 —— 所以启动日志会
显式警告。顶栏的「密钥」会亮成灰色（未设置）；`/api/health` 也会返回
`auth_enabled`，方便探针判断当前实例是否受保护。

启用之后：

| 通道 | 凭据位置 |
|---|---|
| REST `/api/*` | 请求头 `X-API-Key` |
| WebSocket `/ws/training` | 查询参数 `?api_key=...`（浏览器无法给 WS 设请求头） |
| `/api/health` | 豁免 —— 存活探针不该需要凭据 |
| `/api/docs` · `/api/openapi.json` | 豁免 —— Swagger UI 由浏览器直接打开，带不了请求头；真正的调用没有 key 依然 401 |
| 静态资源（SPA 本身） | 豁免 —— 否则浏览器连填 key 的界面都加载不出来 |

浏览器端在顶栏「密钥」里填一次，存入 `localStorage`，之后 REST 自动带头、
WS 自动带查询参数。填完不必刷新页面：健康检查每 5 秒一轮、WS 断开后 2 秒重连，
两者都会重新读取。

!!! tip "401 在界面上显示为「API 需要密钥」"
    服务正常、只是凭据不对时，顶栏不会说「离线」——那会把排查方向带到进程和
    端口上去，而真正要做的是补一个 key。
...
这个决定来自实测：1024px 及以上在 8 种视口 × 6 页的组合探测中零元素溢出（模型注册中心页上线后按 4 种视口复测）；
...
└── src/
    ...
    │   └── ToastHost.vue    # 反馈层宿主
    └── pages/               # 7 个页面 + NotFoundPage（路由懒加载）
```
